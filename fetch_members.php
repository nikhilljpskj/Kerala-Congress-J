...
$sql = "SELECT reg_no, fname, lname, mobile, aadhaar, gender, assembly, selfgovt, photo 
        FROM members 
        WHERE membership = ? AND district = ? AND status = 0";
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="#"><i class="fas fa-id-card"></i> Get Card</a>
                    ...
                    <?php if (!empty($members)): ?>
                        <?php foreach ($members as $member): ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-approve" onclick="approveMember('<?php echo htmlspecialchars($member['reg_no']); ?>')">Approve</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="9" class="text-center">No new members found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <script>
        function approveMember(reg_no) {
            if (confirm('Are you sure you want to approve this member?')) {
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "approve_member.php", true);
                ...
                xhr.send("reg_no=" + encodeURIComponent(reg_no));
            }
        }
    </script>
</body>
